export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query('SELECT * FROM Users WHERE email = ? AND password = ? AND deletedAt IS NULL LIMIT 1', { replacements: [req.body.email || '', security.hash(req.body.password || '')], model: models.User, plain: true })
