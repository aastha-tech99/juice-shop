...
describe('OAuthComponent', () => {
    ...
    it('will create regular user account with base64 encoded reversed email as password', () => {
        ...
        component.ngOnInit()
        expect(userService.save).toHaveBeenCalledWith({ email: 'test@test.com', password: btoa('test@test.com'.split('').reverse().join('')), passwordRepeat: btoa('test@test.com'.split('').reverse().join('')) })
    })
    ...
    it('logs in user even after failed account creation as account might already have existed from previous OAuth login', () => {
        ...
        component.ngOnInit()
        expect(userService.login).toHaveBeenCalledWith({ email: 'test@test.com', password: btoa('test@test.com'.split('').reverse().join('')), oauth: true })
    })
