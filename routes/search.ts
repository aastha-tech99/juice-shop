          ...
          UserModel.findAll().then(data => {
            ...
            if (users.data?.length) {
              for (const userData of users.data) {
                solved = solved && utils.containsOrEscaped(dataString, userData.email) && dataString.includes(userData.password)
                if (!solved) {
                  break
                }
              }
        ...
        if (challengeUtils.notSolved(challenges.dbSchemaChallenge)) {
          ...
          void models.sequelize.query('SELECT sql FROM sqlite_master').then(([data]: any) => {
            ...
            if (tableDefinitions.data?.length) {
              for (const tableDef of tableDefinitions.data) {
                if (tableDef.sql) {
                  solved = solved && utils.containsOrEscaped(dataString, tableDef.sql)
                  if (!solved) {
                    break
                  }
                }
