...
export function login () {
  ...
  function verifyPreLoginChallenges (req: Request) {
    ...
    challengeUtils.solveIf(challenges.oauthUserPasswordChallenge, () => { return req.body.email === 'bjoern.kimminich@gmail.com' && req.body.password === Buffer.from('bjoern.kimminich@gmail.com'.split('').reverse().join('')).toString('base64') })
    challengeUtils.solveIf(challenges.exposedCredentialsChallenge, () => { return req.body.email === 'testing@' + config.get<string>('application.domain') && req.body.password === (process.env.EXPOSED_TESTING_PW || 'testing-pw') })
  }
