...
void describe('/redirect', () => {
  ...
  void it('GET error when calling /redirect without query parameter', async () => {
    const res = await request(app)
      .get('/redirect')
  ...
  void it('GET error when calling /redirect with unrecognized query parameter', async () => {
    const res = await request(app)
      .get('/redirect?x=y')
    assert.equal(res.status, 406)
    assert.ok(res.headers['content-type']?.includes('text/html'))
  ...
  void it('GET error when allowlist bypass is attempted via query string embedding', async () => {
    const res = await request(app)
