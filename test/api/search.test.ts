...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search with SQL injection attempt is prevented by parameterized query', async () => {
    const res = await request(app)
  ...
  void it('GET product search UNION SELECT with closing parenthesis is prevented by parameterized query', async () => {
    const res = await request(app)
      .get("/rest/products/search?q=') union select id,email,password from users--")
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
  ...
  void it('GET product search UNION SELECT * is prevented by parameterized query', async () => {
    const res = await request(app)
  ...
  void it('GET product search UNION SELECT with user columns is prevented by parameterized query', async () => {
    const res = await request(app)
  ...
  void it('GET product search cannot select logically deleted product via SQL injection due to parameterized query', async () => {
    const res = await request(app)
