...
void describe('verify', () => {
  ...
  void describe('jwtChallenges', () => {
    ...
    void it('"jwtUnsignedChallenge" is solved when forged unsigned token has email jwtn3d@juice-sh.op in the payload', () => {
      req.headers = { authorization: 'Bearer eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJkYXRhIjp7ImVtYWlsIjoiand0bjNkQGp1aWNlLXNoLm9wIn0sImlhdCI6MTUwODYzOTYxMiwiZXhwIjo5OTk5OTk5OTk5LCJhdWQiOiJqdWljZS1zaG9wIiwiaXNzIjoianVpY2Utc2hvcCJ9.' }
    ...
    void it('"jwtUnsignedChallenge" is solved when forged unsigned token has string "jwtn3d@" in the payload', () => {
      req.headers = { authorization: 'Bearer eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJkYXRhIjp7ImVtYWlsIjoiand0bjNkQCJ9LCJpYXQiOjE1MDg2Mzk2MTIsImV4cCI6OTk5OTk5OTk5OSwiYXVkIjoianVpY2Utc2hvcCIsImlzcyI6Imp1aWNlLXNob3AifQ.' }
    ...
    void it('"jwtForgedChallenge" is solved when forged token HMAC-signed with public RSA-key has email rsa_lord@juice-sh.op in the payload', { skip: isWindows() ? 'not supported on Windows' : false }, () => {
      req.headers = { authorization: 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJkYXRhIjp7ImVtYWlsIjoicnNhX2xvcmRAanVpY2Utc2gub3AifSwiaWF0IjoxNzkwMjQxNzg3LCJhdWQiOiJqdWljZS1zaG9wIiwiaXNzIjoianVpY2Utc2hvcCJ9.O3D3A0CCsegHu0cYt-DhvZVgLCOtJClikSTpoXnudYI' }
    ...
    void it('"jwtForgedChallenge" is solved when forged token HMAC-signed with public RSA-key has string "rsa_lord@" in the payload', { skip: isWindows() ? 'not supported on Windows' : false }, () => {
      req.headers = { authorization: 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJkYXRhIjp7ImVtYWlsIjoicnNhX2xvcmRAIn0sImlhdCI6MTc5MDI0MTc4NywiYXVkIjoianVpY2Utc2hvcCIsImlzcyI6Imp1aWNlLXNob3AifQ.sEYIsdsKCg9T_mYSHbjCLmAQQtspFBtaBGwb9SxJ1x0' }
