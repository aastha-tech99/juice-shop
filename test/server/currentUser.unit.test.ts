...
void describe('currentUser', () => {
  ...
  void it('should return ID and email of user belonging to cookie from the request', () => {
    // Generate admin token at runtime to avoid hardcoded JWT strings
    const adminToken = authorize({ data: { id: 1, email: 'admin@juice-sh.op', lastLoginIp: '0.0.0.0', profileImage: 'default.svg' } })
